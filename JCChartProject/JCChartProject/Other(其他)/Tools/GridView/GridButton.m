...
-(instancetype)initWithFrame:(CGRect)frame title:(NSString *)title normalImage:(UIImage *)normalImage highlightedImage:(UIImage *)highlightedImage girdId:(NSInteger)gridId atIndex:(NSInteger)index isAddDelete:(BOOL)isAddDelete deleteImage:(UIImage *)deleteImage withIconImageString:(NSString *)iconImageString andObjectId:(NSString *)objectId
{
    self = [super initWithFrame:frame];
    if (self) {
        //计算每个格子的坐标
